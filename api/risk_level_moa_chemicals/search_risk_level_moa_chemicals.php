...
function opt_enum($v, array $allowed, string $code) : ?string {
  ...
  return $s;
    ...
    try {
      ...
      $sql = "SELECT r.id, r.risk_level_id, r.moa_group_id, mg.moa_code, r.chemical_id, c.trade_name, c.active_ingredient, c.target_type, r.priority FROM risk_level_moa_chemicals r INNER JOIN moa_groups mg ON mg.moa_group_id=r.moa_group_id INNER JOIN chemicals c ON c.chemical_id=r.chemical_id WHERE (c.trade_name LIKE :q OR c.active_ingredient LIKE :q OR mg.moa_code LIKE :q OR mg.group_name LIKE :q) AND (:risk_level_id IS NULL OR r.risk_level_id = :risk_level_id) AND (:moa_group_id IS NULL OR r.moa_group_id = :moa_group_id) AND (:chemical_id IS NULL OR r.chemical_id = :chemical_id) ORDER BY r.risk_level_id ASC, r.moa_group_id ASC, r.priority ASC, r.id ASC";
      $st = $db->prepare($sql);
