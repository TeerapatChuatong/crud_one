...
function opt_enum($v, array $allowed, string $code) : ?string {
  ...
  return $s;
    ...
    try {
      ...
      $tmp_chemical_id = require_int($data['chemical_id'] ?? null, 'chemical_id_invalid');
      $tmp_priority = opt_int($data['priority'] ?? null, 'priority_invalid');

      $stmt = $db->prepare("INSERT INTO risk_level_moa_chemicals (risk_level_id, moa_group_id, chemical_id, priority) VALUES (:risk_level_id, :moa_group_id, :chemical_id, :priority)");
      $stmt->execute([
        'risk_level_id' => $tmp_risk_level_id,
        'moa_group_id' => $tmp_moa_group_id,
        'chemical_id' => $tmp_chemical_id,
